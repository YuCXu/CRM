...
import com.xyc.crm.utils.UploadUtils;
...
public class CustomerAction extends ActionSupport implements ModelDriven<Customer> {
	...
	public String findAll() {
		//接收参数，分页查询
		//最好使用 DetachedCriteria对象（条件查询---带分页）
	...
	public String edit() {
		...
		customer = customerService.findById(customer.getCust_id());
		//将customer传递到页面
		//两种方式：一种：手动压栈。二种：因为模型驱动的对象，默认再栈顶
		//如果第一种方式：回显数据：  <s:property value="cust_name"/>  <s:textfield name="cust_name"/>
		//如果第二种方式：回显数据：<s:property value="model.cust_name"/>
		//跳转页面
